...
To rigorously validate the generalisability of the proxy, we conducted a massive parallelised batch experiment testing $N=10,000$ unique configurations with randomized masses ($m_i \in [1.0, 10.0]$), planar starting positions, and random non-zero initial velocities. Across 8,496 valid trajectories integrating up to $T=100$ prior to system ejection, we establish a robust monotonic, nonlinear relationship across generalised topologies, achieving a median Spearman rank of $\rho=0.640$ and median mutual information of $0.645$. The median Pearson correlation is $0.910$ (mean $0.758 \pm 0.345$).

The proxy establishes a structural isomorphism with the finite-time LLE; it captures the macroscopic phase shift triggered by close encounters that the LLE captures at the microscopic divergence level. Because it operates on macroscopic quantities already required to integrate the base state vector, it provides zero-overhead chaos detection.
